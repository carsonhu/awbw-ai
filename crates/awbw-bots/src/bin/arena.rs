...
use awbw_bots::arena::{elo_difference, play_match};
...
fn main() {
    ...
    if args.iter().any(|a| a == "--show-map") {
        ...
        println!("\nupper case = seat 0, lower = seat 1, unmarked = neutral");
    ...
    println!("{games} games per pairing, {max_day}-day cap, seats swapped each game\n");
    ...
    for i in 0..bots.len() {
        for j in (i + 1)..bots.len() {
            ...
            let result = play_match(left[i].as_mut(), right[0].as_mut(), games, max_day);
